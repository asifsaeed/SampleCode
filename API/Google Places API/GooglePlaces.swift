...
public extension GooglePlaces {
  ...
  public static let placeNearbySearchURLString = "https://maps.googleapis.com/maps/api/place/nearbysearch/json"
  
  public class func placeNearbySearch(forLocationCoordinate location: LocationCoordinate2D,
                                      completion: ((PlaceNearbySearchResponse?, NSError?) -> Void)?)
  {
    var requestParameters: [String : Any] = baseRequestParameters + [
    ...
    let request = Alamofire.request(placeNearbySearchURLString, method: .get, parameters: requestParameters).responseJSON { response in
      if response.result.error?._code == NSURLErrorCancelled {
        // nothing to do, another active request is coming
        return
      }
      ...
      // Nil
      if let _ = response.result.value as? NSNull {
        completion?(PlaceNearbySearchResponse(), nil)
        return
      }
      ...
      guard let response = Mapper<PlaceNearbySearchResponse>().map(JSON: json) else {
        NSLog("Error: Mapping directions response failed")
        completion?(nil, NSError(domain: "GooglePlacesError", code: -3, userInfo: nil))
        return
      }
...
// MARK: - Photo Requests
public extension GooglePlaces {
  public static let placePhotoRequestURLString = "https://maps.googleapis.com/maps/api/place/"
  // 300px300px images requested for @3x screens. Ideally this would be configurable
  
  fileprivate static var photoURLFormat: String {
    return GooglePlaces.placePhotoRequestURLString + "photo?key=%@&photoreference=%@&maxwidth=1050&maxheight=1050"
  }
  
  public class func urlForPhoto(reference: String) -> String {
    return String(format: photoURLFormat, "AIzaSyBAz4BXDcIzAjHND4htZs-eleqXJiuFKgs", reference)
  }
